...
void serial_init(serial_t *obj, PinName tx, PinName rx)
{
    ...
    if (tx != NC) {
        LPUART_EnableTx(uart_addrs[obj->serial.index], true);
        pin_mode(tx, PullUp);
    }
    if (rx != NC) {
        LPUART_EnableRx(uart_addrs[obj->serial.index], true);
        pin_mode(rx, PullUp);
    }
...
{
    LPUART_Deinit(uart_addrs[obj->serial.index]);
    serial_irq_ids[obj->serial.index] = 0;
}

void serial_baud(serial_t *obj, int baudrate)
{
    LPUART_SetBaudRate(uart_addrs[obj->serial.index], (uint32_t)baudrate, CLOCK_GetFreq(uart_clocks[obj->serial.index]));
}

void serial_format(serial_t *obj, int data_bits, SerialParity parity, int stop_bits)
{
    LPUART_Type *base = uart_addrs[obj->serial.index];
    uint8_t temp;
...
void serial_irq_set(serial_t *obj, SerialIrq irq, uint32_t enable)
{
    ...
    if (enable) {
        switch (irq) {
            case RxIrq:
                LPUART_EnableInterrupts(uart_addrs[obj->serial.index], kLPUART_RxDataRegFullInterruptEnable);
                break;
            case TxIrq:
                LPUART_EnableInterrupts(uart_addrs[obj->serial.index], kLPUART_TxDataRegEmptyInterruptEnable);
                break;
            default:
                break;
        }
        ...
        switch (irq) {
            case RxIrq:
                LPUART_DisableInterrupts(uart_addrs[obj->serial.index], kLPUART_RxDataRegFullInterruptEnable);
                break;
            case TxIrq:
                LPUART_DisableInterrupts(uart_addrs[obj->serial.index], kLPUART_TxDataRegEmptyInterruptEnable);
                break;
            default:
                break;
        }
        switch (other_irq) {
            case RxIrq:
                all_disabled = ((LPUART_GetEnabledInterrupts(uart_addrs[obj->serial.index]) & kLPUART_RxDataRegFullInterruptEnable) == 0);
                break;
            case TxIrq:
                all_disabled = ((LPUART_GetEnabledInterrupts(uart_addrs[obj->serial.index]) & kLPUART_TxDataRegEmptyInterruptEnable) == 0);
                break;
            default:
                break;
        }
...
int serial_getc(serial_t *obj)
{
    ...
    LPUART_ReadBlocking(uart_addrs[obj->serial.index], &data, 1);
    return data;
}
...
void serial_putc(serial_t *obj, int c)
{
    ...
    LPUART_WriteByte(uart_addrs[obj->serial.index], (uint8_t)c);
}

int serial_readable(serial_t *obj)
{
    uint32_t status_flags = LPUART_GetStatusFlags(uart_addrs[obj->serial.index]);
    if (status_flags & kLPUART_RxOverrunFlag)
        LPUART_ClearStatusFlags(uart_addrs[obj->serial.index], kLPUART_RxOverrunFlag);
...
{
    uart_addrs[obj->serial.index]->CTRL |= LPUART_CTRL_SBK_MASK;
}
